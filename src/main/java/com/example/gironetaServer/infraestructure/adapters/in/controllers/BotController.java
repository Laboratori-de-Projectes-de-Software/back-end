...
public class BotController {
    ...
    public ResponseEntity<BotResponseDTO> createBot(@RequestBody BotDto botDto) {
        ...
        return new ResponseEntity<>(BotMapper.toBotResponseDto(savedBot), HttpStatus.OK);
    }
    ...
    // /bot?owner={userid}
    @GetMapping("/bot")
    public ResponseEntity<List<BotResponseDTO>> getBots(@RequestParam(name = "owner", required = false) Long userId) {
        List<Bot> bots;
        ...
        List<BotResponseDTO> botResponseDTO = bots.stream()
                .map(BotMapper::toBotResponseDto)
                .collect(Collectors.toList());

        return ResponseEntity.ok(botResponseDTO);
    }
