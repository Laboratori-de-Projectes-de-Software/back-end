    ...
    public void checkFieldViolations(){
        ...
        if (!violations.isEmpty()) {
            ...
            for (ConstraintViolation<LeagueRequest> v : violations) {
                ...
                switch (campo) {
                    case "name":
                        messageError.append(messageConverter.getMessage(Message.NOT_NAME, null, this.locale));
                        break;
                    case "numRounds":
                        messageError.append(messageConverter.getMessage(Message.NOT_ROUNDS, null, this.locale));
                        break;
                    case "playTime":
                        messageError.append(messageConverter.getMessage(Message.NOT_PLAYTIME, null, this.locale));
                        break;
                    default:
